...
recognizeDemoApp =  {
    ...
    enrollImage: function(input) {
        ...
        if ($(".enrolled-image").length >= self.enrollmentLimit ) {
            ...
            self.getTemplate("image-left-template","You have reached the image enrollment limit.",false,false);
            setTimeout(function(){
            ...
            if (!fileTypeAllowed) {
                ...
                self.getTemplate("image-left-template",filetypeMsg,false,false);
                setTimeout(function(){
                    $(".image-left-template").fadeOut(); 
                },3000);
                return false;
                ...
                self.getTemplate("image-left-template","Analyzing image...","",true);
                self.getTemplate("image-right-template","","",false);
    ...
    recognizeImage: function(input) {
        ...
        if (!$('.main-container').hasClass("enrolled")) {
            self.getTemplate("image-right-template","You must first enroll at least one image on the right",false,false);
        }
        else {
            var ftype = file.type; // get file type
            var fileTypeAllowed = false;
            ...
            if (!fileTypeAllowed) {
                var filetypeMsg = "Wrong file type.  Must be" + fileTypeList;
                self.getTemplate("image-right-template",filetypeMsg,false,false);
                setTimeout(function(){
                ...
                self.getTemplate("image-left-template","","",false);
                self.getTemplate("image-right-template","Analyzing image...","",true);
                var reader  = new FileReader();
                reader.readAsDataURL(file);
                reader.onloadend = function () {
                    var imageData = String(reader.result);
    ...
    displayResponse: function(response, process, subjectId) {
        if (process == "enroll") {
            ...
            if (response.Errors) {
                self.getTemplate("image-left-template","Error: " + utils.toTitleCase(response.Errors[0].Message),"",false);
                $(".image-right-template").fadeOut(); 
                setTimeout(function(){
                    $(".image-left-template").fadeOut(); 
                    $(".show-json").hide(); 
            ...
            $(".left-image-container .user-instructions").hide();
            if (response.Errors) {
                self.getTemplate("image-right-template","Error: " + utils.toTitleCase(response.Errors[0].Message),"",false);
                setTimeout(function(){
                    $(".show-json").hide(); 
                ...
                if (recognized) {
                    self.getTemplate("image-right-template","RECOGNIZED","",false);
                }
                else {
                    self.getTemplate("image-right-template","NOT RECOGNIZED","",false);
                }
                utils.showJsonResponse(response)
            }
        }
