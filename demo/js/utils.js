...
utils =  {
    ...
    //------------------------------------
    // CHECK MIME TYPE - CALLBACK IF NOT IN CONFIG ARRAY
    //------------------------------------ 
    checkMimeType: function(config, input, origin, callback, position) {
        ...
        $.ajax({
            ...
            var mimeType = response.fileType.split(";")[0];
            var fileTypeList = [];
            $(config.uploadFileTypesImage).each(function(idx, fileType) {
