...
function MediaStreamRecorder(mediaStream) {
    ...
    this.blobExists = function() {
        return mediaRecorder.blobs[0];
    },

    this.saveToDir = function(file, fileName) {
        if (!file) {
            if (!mediaRecorder) {
                return;
            }
...
function WhammyRecorderHelper(mediaStream, root) {
    this.record = function(timeSlice) {
        ...
        if (this.video && this.video instanceof HTMLVideoElement) {
            if (!this.width) {
                this.width = video.videoWidth || video.clientWidth || 640;
            }
            if (!this.height) {
                this.height = video.videoHeight || video.clientHeight || 480;
            }
        }
...
function GifRecorder(mediaStream) {
    ...
    this.start = function(timeSlice) {
        ...
        var imageWidth = this.videoWidth || 640;
        var imageHeight = this.videoHeight || 480;
