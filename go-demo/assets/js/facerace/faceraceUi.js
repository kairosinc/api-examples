...
window.__enable_share_if_loads = 1;
...
function getSha(url) {
  var sha = (url || '').match(/(.*\/facerace\/.*\/|sha=)([a-z0-9]{10,40})/);
  var sha = sha && sha[2] || '';
  return sha;
}

function generateTwitterShare(url) {
  var env = window.__e || 'dev';
  var demo_url = 'https://'+(env=='prod'?'':env.replace("stage","staging")+'.')+'kairos.com/twitter_card.php?via=twitter&i=' + getSha(url); // via Higgs
